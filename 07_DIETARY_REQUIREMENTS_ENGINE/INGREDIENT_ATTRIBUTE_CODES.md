...
| `LACTOSE_CONTENT` | Contains lactose. Added in v1.2 (Phase 2.5 addendum). Distinct from `DAIRY_MILK`: a lactose-free/lactose-reduced dairy product is `DAIRY_MILK=true` (still carries the milk allergen) but `LACTOSE_CONTENT=false`. In this table, every dairy ingredient mirrors its own `DAIRY_MILK` value here, because the V1 recipe bank only ever names ordinary dairy; the distinction becomes load-bearing once a substitution catalogue offers "lactose-free milk" as a real option (`04_PRODUCTION_STARTER/src/dietary/substitutionCatalogue.ts`). `LACTOSE_FREE` classification reads this code, not `DAIRY_MILK`. |
